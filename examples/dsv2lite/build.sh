...
mkdir -p "$OUT"
INLINE="${ETX_INLINE_TILES:-1}"     # 1: single-TU kernel (tile bodies included and inlinable); 0: separate shim TU
if [ "$INLINE" = "1" ]; then
  "$ROOT/.venv/bin/python" -m etx compile examples/dsv2lite/graph.py --arch gfx942 --out "$OUT" --inline-tiles | grep -E "tasks=|workers/domain"
  SHIM=""
else
