...
| M3 Match 3.60 ms | Per-phase trace on both kernels; close the gaps until ETX is within 3% of fleet | 3.60 ms per token, 32/32 tokens | In progress: 3.70 ms/token vs fleet's 3.556 on the same VM (4% gap; was 4.09 / 15%) after four fixes (section 15.4); remaining: the per-barrier transition (~2 us x 8 per layer) and the qkv body (+4 us) |
