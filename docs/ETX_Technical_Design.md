...
| M3 Match 3.60 ms | Per-phase trace on both kernels; close the gaps until ETX is within 3% of fleet | 3.60 ms per token, 32/32 tokens | 3.76-3.78 ms/token vs fleet's 3.557 on the same GPU with the safe runtime (6% gap; 3.70 / 4% measured with the relay's hierarchical acquire, later shown unsafe, section 15.5) after four fixes (section 15.4); remaining: the per-barrier transition (~2 us x 8 per layer) and the qkv body (+4 us) |
...
| + one call site per tile body | the emitter now splits each task into a per-type wait switch, ONE body call, and a per-type arrive switch; before, the body dispatch was inlined into each of the 219 per-layer task types, so the compiler stopped inlining and paid the call ABI | 3.90 (with the next three rows) | 32/32, 27/27 | accumulation registers 180 -> 47 |
| + lean static loop, layer immediates | an all-static device gets a loop without the readiness probe and queue pops (one poller and one round trip less per task); per-grid constants (`TaskGrid.consts`, `ctx.cst`) hand the layer index to the body as an immediate instead of two dependent loads after the wait | 3.90 | 32/32 | A/B against the 4.10 build on the same VM |
| + per-domain relay, hierarchical acquire | P5 reserves one workgroup per XCD to mirror the DEVICE-scope counters (fleet's scheduler mirror, chosen by a cost-table threshold of 64 pollers per word); the relay invalidates its XCD's L2 once and consumers drop only their L1 | 3.90 (relay on) vs 3.88 (off) | 32/32 | neutral on its own; with the relay but per-consumer full acquire 4.04, so the hierarchical acquire is what pays for the relay hop |
| + no `-fgpu-rdc` | whole-program device compile, as fleet builds; a non-inlined relay taking `etx_params&` had also spilled the parameter block to scratch (fixed by force-inlining) | **3.70** | 32/32, 27/27 | scratch 112 -> 0 B/lane; sweeps: `s_sleep` 1 / 3 / 8 = 3.74 / 3.73 / 3.71; 38 workers without relay 3.78; descriptors staged in LDS 3.72 (reverted) |
...
Second round (2026-09-23 evening, fresh VM; old build re-measured at 4.10, fleet 3.556): layer span 130.8 us traced vs fleet's 126.9; every tile body now matches or beats fleet's (qkv 18.6 vs 17.1, attention 13.0, merge 8.6 vs 9.1, o_proj 6.0 vs 6.9, router 8.5 vs 10.7, gate_up 31.3 vs 33.6, down 16.1 vs 18.0 us). The remaining ~4 us per layer is in three handoffs (after o_proj, after the router, after down), each 2-3 us where fleet shows about 1 us; fleet also keeps 32 tokens in one launch, which ETX's per-token launch does not. Full A/B log: `examples/dsv2lite/ab_2026-09-23.log`.

Third round (2026-09-24, fresh VM; fleet 3.550-3.559 interleaved in the same A/B): per-domain shares as immediates in the last-arriver flush (3.711-3.719 vs 3.699-3.719 before), a relay that spins without backoff (3.728-3.731, worse, off), and `__launch_bounds__` carrying the plan's workgroups per CU (3.726-3.728, neutral) all measured within noise. A full-token trace alignment located the gap: the dense layer 0 is faster in ETX (119 vs 122 us), the 26 MoE layers are 6.1 us slower each (133.0 vs 126.9), and embed + lm_head + argmax add about 17 us. Per XCD, the router -> gate_up and down -> qkv handoffs match fleet once the stores' drain is counted on the same side; what remains is the qkv body (about 2 us, fleet's own code compiled in a different kernel) and the merge -> o_proj handoff (about 1.5 us). Log: `examples/dsv2lite/ab2_2026-09-24.log`.

Three lessons for the design itself. First, a microbenchmark-optimal primitive can be wrong under load: the inv-L1 poll was chosen because it was faster idle and it cost 5% end to end; the cost table now records both figures and the poll row cites the loaded measurement. Second, "one fence per workgroup" is a rule the code generator must own: a tile author cannot see that the generated prologue executed the acquire on every wave. Third, code shape is part of the performance contract: a dispatch that gives a body hundreds of call sites, or one non-inlined helper that takes the parameter block by reference, costs more than any synchronisation choice measured here (0.2 ms/token each).

### 15.5 M4 on hardware: three more models, no model-specific runtime code (2026-09-24, 2x MI300X VM)

`examples/llm` builds the ETX graph from a Hugging Face `config.json` (dense GQA with optional QKV bias and per-head q/k norm, tied or untied embeddings, and Qwen3-MoE routing) and runs one set of generic batch-1 tiles: RMSNorm-prologue GEMV, q/k post-processing with RoPE and the KV-cache write, chunked GQA attention with a merge, router, expert gate_up and down, and lm_head with argmax. Each expert task recomputes the token's top-k from the router logits, so there is no separate top-k phase. Values are rounded to bf16 wherever Hugging Face materialises a bf16 tensor. The reference is Hugging Face greedy decoding (bf16, transformers 4.57.6, torch 2.10 ROCm 7.0) on the same prompt, unconstrained, 32 tokens.

| Model | Structure | Tasks per token | Teacher-forced argmax vs HF | Free-running tokens vs HF | ms/token | Weight GB/s |
|---|---|---|---|---|---|---|
| Qwen2.5-1.5B | dense, GQA 12/2, QKV bias, tied embeddings | 27.7 k | 31/32, the miss at an exact HF tie (top-1 margin 0.000) | first 24/32, diverging at that tie | 5.11 | 605 |
| Qwen3-8B | dense, GQA 32/8, q/k norm | 45.5 k | 32/32 | 32/32 | 13.4 | 1,130 |
| Qwen3-30B-A3B | MoE, 128 experts, top-8, GQA 32/4 | 102.9 k | 32/32 | 32/32 | 14.4 | 422 (active weights) |

Every result is deterministic over three runs. The compiler chose static scheduling for every grid of every model, with two workgroups per CU for the MoE model where the LDS allowed it. The tiles are general, not tuned: the per-phase trace shows the time in GEMV bodies that stay latency-bound on small tasks, and in attention at short context, not in synchronisation. Two tile changes measured on the same VM: split K across waves when a task has fewer than 8 rows, else give rows to waves (MoE 16.9 to 14.4 ms); attention chunks of 32 positions instead of 128.
